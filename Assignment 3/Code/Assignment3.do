...
global wd "/Users/amylim/Library/CloudStorage/OneDrive-EmoryUniversity/Econ 771/Assignment 3/"
...
***PART 1: Prepare program and supplemental data
	...
	***Prepare some supplemental data
		use "$wd/Data/Data_subsidyinfo.dta", replace
...
/*		
estpost tabstat premium deductible EBene thisPlansExist_F0 thisPlansExistState_F0 , c(stat) stats(mean sd N) by(year) nototal

esttab using "$wd/Output/table1.tex", replace ///
  main(mean %8.0fc) aux(sd %8.0fc) nostar nonumber unstack ///
   compress nonote noobs gap label booktabs   ///
   collabels(none) ///
   eqlabels("2006" "2007" "2008" "2009" "2010") ///  
   nomtitles
est clear
** for the counts of unique firms and plans, i had to manually tabulate and add: 
*unique firms:
tab tagfirmID cohort 
*number of plans, values from e(count) 
*/
*#2
...
/***Figure 3: RD-Enrollment (with new bins)
		***Algorithm to computer N bins with range +/- h
			local nBinOver2 = 30
			local h = 10
			...
			***Regress to get line: from quartic
			...
			local ytitle = "Log Enrollment Share, 2006"		
			twoway (scatter lnSHat theBinAl if year ==`year' & RDwindow2006`ScatWin'==1 & `P'benefit =="B") (line lnSHatAlt `P'LISPremium if year ==`year' & RDwindow2006`RegWin'==1 & `P'benefit =="B",sort lpattern(dash) lcolor(gray)) (line lnSHatAltPoly `P'LISPremium if year ==`year' & RDwindow2006`PolyWin'==1 & `P'benefit =="B",sort lpattern(solid) lcolor(black)), legend(order(2 3) label(2 "Local Linear") label(3 "Quartic Polynomial")) xtitle("Monthly Premium - LIS Subsidy, 2006 (Partition=10)") ytitle("`ytitle'") 
*/
*#4 
rdrobust lnS theBinAl 
rdplot lnS  theBinAl, binselect(es) 
...
*****PART 3C: Creates RD Tables and Figures
	***Creates Table 3
		...
		***Set bandwidth
		...
		outreg2 [iS??Lin*] using "$wd/Output/Table3-PanelA.xlsx", replace 
		outreg2 [iS??Opt*] using "$wd/Output/Table3-PanelB.xlsx", replace 
		outreg2 [intSLin*] using "$wd/Output/Table3-PanelC.xlsx", replace 
	
		capture estimates drop *

*#7
rdrobust lnS theBinAl, bwselect(certwo)

*#8
gen ind=cond(LISsubsidy2006Temp >0,1,0)
eststo: reghdfe lnS ind theBinAl, absorb(firmID) cluster(firmID) 
esttab, b se nomtitle label star(* 0.10 ** 0.05 *** 0.01)
esttab  using "$wd/Output/table8.tex", replace  ///
 b se nomtitle label star(* 0.10 ** 0.05 *** 0.01)  ///
 booktabs alignment(D{.}{.}{-1}) ///
 title(regression table \label{reg1})   ///
 scalars( "N Obs." "Time FE" r2) sfmt(3) ///
 addnotes("Dependent variable: Log Market Share")
 est clear
